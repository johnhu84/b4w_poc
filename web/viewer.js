"use strict";
var m_scenes, data_id, _selected_object;
var obj_arr = [], funcs = [];//, obj_ids = [], obj_names = [];
...
function init_cb(canvas_elem, success) {
    ...
    _object_selected_callback = function(obj) {
   	...
   	//$('#change_diffuse_map').parent().removeClass('ui-disabled'); 
...
//var funcs = [];
    for (var i in objs) {
	var file_name = objs[i];
	/*funcs[i] = function(data_id) {
		obj_ids[j] = data_id;
		obj_names[j++] = file_name;
		enable_physics();
		return file_name;
	};*/
	var file_name = objs[i];
	if (!objs[i].endsWith("environment.json") && !objs[i].endsWith("background.json"))
		obj_names.push(objs[i]);	
		obj_arr.push(m_data.load(objs[i], function(data_id) {
			obj_ids[j++] = data_id;
			enable_physics();
		}, null, null, true));
    }
...
function main_canvas_clicked(x, y) {
    ...
    if (!m_scenes.can_select_objects()) 
        return;
    ...
    $('#data_id').html(m_scenes.get_object_data_id(obj));
    if (obj) {
...
function process_scene(url, call_reset_b4w) {
    ...
    // load
    data_id = m_data.load(url, loaded_callback, preloader_callback, false);
